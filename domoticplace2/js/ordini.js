...
function get_coll(coll) {
    var collection = db.collection(coll);
    var confronto = new Array("o","r","d","i","n","e");
    for (let i = 0; i < 7; i++) {
...
function registrati(){
    ...
    //controllo se esiste già l'account
    var collection = db.collection('tutto');
    var trovati=0;
    collection
        .get()
        .then(function(querySnapshot) {
        ...
        if(trovati==1){
            document.getElementById('controllo').style.color = "red";
            document.getElementById('controllo').innerHTML = "<p>Account già esistente</p><a class='recupera' onclick='recupera()'>recupera password</a>" ;
        }
    });

    insert_doc("tutto", user, { 
        email: String(email),
        pass: String(pass)
      });
}
